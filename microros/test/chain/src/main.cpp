...
std_msgs__msg__Int32 angle_order;
...
void setup() {
  ...
  RCCHECK(rclc_subscription_init_default(
    ...
    &node,
    ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32),
    "micro_ros_node_subscriber"));

  // create executor
  RCCHECK(rclc_executor_init(&executor, &support.context, 1, &allocator));
  RCCHECK(rclc_executor_add_subscription(&executor, &subscriber, &angle_order, &subscription_callback, ON_NEW_DATA));
}
