...
void OmuniDirWalk::update_joy_order(const sensor_msgs::msg::Joy msg){
    ...
    this->theta = THETA_MAX*msg.axes[2];
}
...
void OmuniDirWalk::traj_to_motion()
{
    ...
    while(!body_to_fixed_foot_traj.empty() && !body_to_swing_foot_traj.empty()){
        if (step_dir == 1){
            ...
            Kinematics::inverse(&foot_roll_right, (*body_to_fixed_foot_traj.begin()), R_ref);
            Kinematics::inverse(&foot_roll_left,  (*body_to_swing_foot_traj.begin()), R_ref);
        }else{
            // fixed... left, swing... right
            Kinematics::inverse(&foot_roll_left,  (*body_to_fixed_foot_traj.begin()), R_ref);
            Kinematics::inverse(&foot_roll_right, (*body_to_swing_foot_traj.begin()), R_ref);
        }
