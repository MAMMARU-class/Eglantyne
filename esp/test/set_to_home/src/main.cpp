...
void loop() {
  ...
  for (int id=0; id<link_set.size(); id++){
    link_set[id]->setq(current[id]);
    link_set[id]->move();
  }
  
  int step_end = 15 * 1000 / 20;
  for(int step=0; step<step_end; step++){
    for(int id=0; id<link_set.size(); id++){
      double pos_id = ( current[id] * (double)(step_end-step) + home[id] * (double)(step) ) / (double)(step_end);
      link_set[id]->setq(pos_id);
      link_set[id]->move();
    }
  }
...
void setup_link(){
  ...
  link_set = {&arm_pitch_right, &arm_roll_right, &hand_right,
