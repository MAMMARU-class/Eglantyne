...
std::vector<RobotLink*> Kinematics::showFromBody(RobotLink* link){
    if (link->parent() == nullptr){
        return {};
    }

    std::vector<RobotLink*> link_list = showFromBody(link->parent());
    link_list.push_back(link);
    return link_list;
}

// forward kinematics. calclate every link's (from body to endeffect) P_w nad R_w
void Kinematics::forward(RobotLink* link){
    if (link->parent() == nullptr){
        Matrix4d RootMat;
...
void Kinematics::inverse(RobotLink* link, Vector3d P_ref, Matrix3d R_ref){
    std::vector<RobotLink*> link_list = showFromBody(link);
...
MatrixXd Kinematics::calcJacobi(std::vector<RobotLink*> link_list){
...
VectorXd Kinematics::calcerr(RobotLink* link, Vector3d P_ref, Matrix3d R_ref){
    VectorXd errVec(6);
