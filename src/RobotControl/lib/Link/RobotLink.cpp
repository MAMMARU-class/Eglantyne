...
void RobotLink::setq(double q){
    // ignore under 0.02 deg
    if (abs(q) < 3e-4) { q = 0; }
    q = std::floor(q*10000) / 10000.0;
...
double RobotLink::getq_min(){return this->q_min;}
double RobotLink::getq_max(){return this->q_max;}
...
// motor_control
void RobotLink::move(double q_order){
    if (abs(q_order) < 3e-4) { q_order = 0; }
    q_order = std::floor(q_order*10000) / 10000.0;
    // if( q_order < q_min ){ q_order = q_min;
    // }else if( q_order > q_max ){ q_order = q_max; }
    motor->setPos(motor_id, rad2pos(q_order));
}
void RobotLink::move_q(){ motor->setPos(motor_id, rad2pos(this->q)); }
