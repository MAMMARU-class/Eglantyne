...
void Link::setState_w(Matrix4d State){
    this->P_w = State.block<3,1>(0,0);
    this->R_w = State.block<3,3>(0,1);
}
Matrix4d Link::getState_w(){
    /*  P_w, R_w
        1,   0,0,0  */
    Matrix4d State_w;
    State_w.block<3,1>(0,0) = this->getP_w();
    State_w.block<3,3>(0,1) = this->getR_w();
    State_w.block<1,3>(3,1) = MatrixXd::Zero(1,3);
    State_w(3,0) = 1;
    return State_w;
}
